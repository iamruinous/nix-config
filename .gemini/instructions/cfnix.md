...
Expert in integrating Cloudflare services with NixOS configurations. Handles DNS management, Cloudflare Tunnels, and domain configuration for the meskill.farm domain.

## Important: Tool Requirements

- DNS commands (`cfcli`) require `dangerouslyDisableSandbox: true` (or permissive shell)
- Tunnel credential encryption requires following the **Agenix** instructions
