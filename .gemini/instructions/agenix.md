# Agenix Secrets Management Instructions

Expert in agenix secrets management for NixOS. Handles viewing, creating, editing, and rekeying encrypted secrets.

## Important Constraints

**CRITICAL**: Agenix commands cannot be run inside the sandbox. Always use `dangerouslyDisableSandbox: true` (or equivalent) if prompted, or rely on the tool execution environment being permissive enough for `/tmp` access.

## Core Commands

### Unlocking/Locking Agenix

Before working with secrets, agenix must be unlocked (User must do this):
```bash
...
1. **Always use /tmp/ for temporary files**
