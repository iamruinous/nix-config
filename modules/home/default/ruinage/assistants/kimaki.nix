...
#
# Example:
#   ruinous.ruinage.assistants.kimaki = {
#     enable = true;
...
with lib; let
  ...
  autoDiscoveredProjects = filterAttrs (name: project:
    (project.namespaces.kimaki.enable or false)
  ) ruinageCfg.projects;

  # Merge auto-discovered projects with explicitly configured ones
  allProjects = cfg.projects // autoDiscoveredProjects;
in {
  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    (mkIf (allProjects != {}) {
      home.activation.registerKimakiProjects = lib.hm.dag.entryAfter ["writeBoundary"] ''
        ...
        '') (attrNames allProjects)}
      '';
    })

    # Auto-inject direnv snippets into .envrc.local
    (mkIf (allProjects != {} && cfg.direnv.autoInject) {
